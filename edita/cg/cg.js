...
function deshabilita(idmc) {
   ...
   $('#indicadores6').hide(1000);
   $('#deshabilita').show(1000);
}
   
function deshabilitaEnvia(idmc){
   $.ajax({
	  url : "deshabilitacg.php",
	  type: "POST",
	  data : {func:"des",idCG:idmc,motivo:$('#formaEnviaDeshabilita').val(),usuario:"1"},
...
function habilitaEnvia(idmc) {
   $.ajax({
	  url : "deshabilitacg.php",
	  type: "POST",
	  data : {func:"hab",idCG:idmc,motivo:$('#formaEnviaDeshabilita').val(),usuario:"1"},
	  success: function(data, textStatus, jqXHR)
	  {
		  location.reload();
	  },
